...
def parse_args():
    ...
    parser.add_argument("--shave", type=int, default=5)
...
def sample(net, device, dataset, cfg):
    ...
    for step, (hr, lr, name) in enumerate(dataset):
        if "DIV2K" in dataset.name:
            ...
            #detach() break the reversed transformation.
    ...
    print (mean_psnr2,  mean_ssim,t2-t1)
        #print '-------------------'z
...
def main(cfg):
    ...
    state_dict = torch.load(cfg.ckpt_path,map_location='cuda:0')
