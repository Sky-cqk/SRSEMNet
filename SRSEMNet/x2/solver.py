...
class Solver():
    ...
    def fit(self):
        ...
        refiner = nn.DataParallel(self.refiner, 
                                  device_ids=range(cfg.num_gpu))#如果可以多个gpu并行计算
        ...
        while True:
            for inputs in self.train_loader:
                ...
                loss.backward()
                nn.utils.clip_grad_norm(self.refiner.parameters(), cfg.clip) #tcw it is drop out, which can prevent overfitting.#一种解决梯度爆炸的方法，clip越大，解决的方式越柔和，clip越小，解决的越暴力
                self.optim.step()
                ...
                self.step += 1
                if cfg.verbose and self.step % cfg.print_interval == 0:#输出一些模型的信息，比如psnr等
                    if cfg.scale > 0:
    ...
    def evaluate(self, test_data_dir, scale=4, num_step=0):#定义评估
        ...
        for step, inputs in enumerate(test_loader): #step is the number of test images
            ...
            # split large image to 4 patch to avoid OOM error#就是避免显卡内存占用不够的问题
    ...
    def decay_learning_rate(self):#随着epoch改变学习率
