...
def random_crop(hr, lr, size, scale):#随机裁剪
...
def random_flip_and_rotate(im1, im2):#定义随机翻转和旋转
...
class TrainDataset(data.Dataset):#训练集定义
...
class TestDataset(data.Dataset):
    def __init__(self, dirname, scale):
        ...
        self.name  = dirname.split("/")[-1]#dirname中最后一个“/”的剩下内容
        self.scale = scale
